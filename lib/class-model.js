           ...
           JNIEnv * env)
{
  ...
  if (jvmti != NULL)
  {
    ...
    for (i = 0; i != method_count; i++)
    {
      ...
      /* Not a member: it is reached through $clinit. */
      if (strcmp (name, "<clinit>") == 0)
        model->class_initializer = method;
      else
  ...
  else if (art_api.available)
  {
    ...
    for (i = 0; i != n; i++)
    {
      ...
      {
        /* ArtMethod::IsClassInitializer() is IsConstructor() && IsStatic(). */
        if (is_static)
          model->class_initializer = id;
        continue;
      }
...
jmethodID
model_find_class_initializer (Model * self)
{
  return self->class_initializer;
}
                                ...
                                ArtClass * klass)
{
  ...
  for (i = 0; i != n; i++)
  {
    ArtMethod * method;
    guint32 access_flags;
    jboolean is_constructor, is_static;
    ...
    if (ctx->include_signature)
    {
      ...
      if (is_constructor)
      {
        g_string_append (name, is_static ? "$clinit" : "$init");
    ...
    {
      if (is_constructor)
        bare_method_name = is_static ? "$clinit" : "$init";
      else
...
export default class Model {
  ...
  findClassInitializer () {
    const id = cm.findClassInitializer(this.handle);
    return !id.isNull() ? id : null;
  }
...
function compileModule (env) {
  ...
  return {
    ...
    findClassInitializer: new NativeFunction(cm.model_find_class_initializer, 'pointer', ['pointer'], fastOptions),
