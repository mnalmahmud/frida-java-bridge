...
EnvJvmti.prototype.getClassLoader = proxy(57, 'int32', ['pointer', 'pointer', 'pointer'], function (impl, klass, classLoaderPtr) {
  const result = impl(this.handle, klass, classLoaderPtr);
  checkJniResult('EnvJvmti::getClassLoader', result);
});

EnvJvmti.prototype.getMethodDeclaringClass = proxy(65, 'int32', ['pointer', 'pointer', 'pointer'], function (impl, method, declaringClassPtr) {
  const result = impl(this.handle, method, declaringClassPtr);
  checkJniResult('EnvJvmti::getMethodDeclaringClass', result);
});
...
EnvJvmti.prototype.getStackTrace = proxy(104, 'int32', ['pointer', 'pointer', 'int', 'int', 'pointer', 'pointer'], function (impl, thread, startDepth, maxFrameCount, frameBuffer, countPtr) {
  const result = impl(this.handle, thread, startDepth, maxFrameCount, frameBuffer, countPtr);
  checkJniResult('EnvJvmti::getStackTrace', result);
});
