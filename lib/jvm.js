...
const jvmtiFrameInfoSize = 16; // jmethodID plus jlocation, the latter a jlong on every ABI
...
export function ensureClassInitialized (env, classRef) {
  // get_method_id() initializes the class before looking anything up, and every class inherits
  // Object.hashCode(), so the lookup succeeds and nothing is thrown.
  const method = env.getMethodId(classRef, 'hashCode', '()I');
  if (method.isNull()) {
    // Interfaces are the one exception, as lookup_method() and its walk of the superinterfaces
    // never reach Object's. They are initialized all the same, before the lookup fails.
    env.exceptionClear();
  }
}

export function findClassWithoutInitializing (env, name) {
  const loader = getCallerClassLoader(env);
  try {
    return env.findClassWithoutInitializing(name, (loader !== null) ? loader : env.systemClassLoader());
  } finally {
    if (loader !== null) {
...
/*
 * jni_FindClass() resolves against the loader of the topmost Java frame on the calling thread,
 * falling back to the system one when there is none -- as there is not whenever we are called
 * from outside a hook. Class.forName() insists on being told, so work out the same loader.
 *
 * Returns a local reference, NULL meaning the bootstrap loader, or null when there is no frame
 * to take one from.
 */
function getCallerClassLoader (env) {
  const { jvmti } = getApi();

  const frames = Memory.alloc(jvmtiFrameInfoSize);
  const numFrames = Memory.alloc(jsizeSize);
  jvmti.getStackTrace(NULL, 0, 1, frames, numFrames);
  if (numFrames.readS32() === 0) {
    return null;
  }

  const result = Memory.alloc(pointerSize);
  jvmti.getMethodDeclaringClass(frames.readPointer(), result);
  const klass = result.readPointer();
  try {
    jvmti.getClassLoader(klass, result);
