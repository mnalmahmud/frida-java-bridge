...
/*
 * Same, but for the JVM, whose FindClass() also initializes -- running the very class initializer
 * that hooking $clinit exists to intercept. Class.forName() stops short of that, but stops short
 * of linking too, and JVMTI will not enumerate the members of an unprepared class, so ask for the
 * declared methods to link it.
 *
 * Takes the name in dotted form, as Class.forName() does, and the loader to resolve it against --
 * FindClass() picks one implicitly, so it is up to the caller to pick the same.
 */
Env.prototype.findClassWithoutInitializing = function (name, loader) {
  const Class = this.javaLangClass();
  const nameValue = this.newStringUtf(name);
  try {
    const handle = this.staticVaMethod('pointer', ['pointer', 'int', 'pointer'])(
      this.handle, Class.handle, Class.forName, nameValue, 0, loader);
    this.throwIfExceptionPending();

    const methods = this.vaMethod('pointer', [])(this.handle, handle, Class.getDeclaredMethods);
    this.throwIfExceptionPending();
    this.deleteLocalRef(methods);

    return handle;
  } finally {
    this.deleteLocalRef(nameValue);
  }
};
  ...
  if (javaLangClass === null) {
    ...
    try {
      ...
      javaLangClass = {
        ...
        forName: this.getStaticMethodId(handle, 'forName',
...
let systemClassLoader = null;
Env.prototype.systemClassLoader = function () {
  if (systemClassLoader === null) {
    const handle = this.findClass('java/lang/ClassLoader');
    try {
      const getSystemClassLoader = this.getStaticMethodId(handle, 'getSystemClassLoader',
        '()Ljava/lang/ClassLoader;');
      const loader = this.staticVaMethod('pointer', [])(this.handle, handle, getSystemClassLoader);
      this.throwIfExceptionPending();
      systemClassLoader = register(this.newGlobalRef(loader));
      this.deleteLocalRef(loader);
    } finally {
      this.deleteLocalRef(handle);
    }
  }
